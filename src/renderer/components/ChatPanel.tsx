...
export default function ChatPanel() {
  ...
  return (
    <>
      <div className="w-80 h-full flex flex-col bg-[#111118] border-l border-white/10">
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
          <span className="text-sm font-bold tracking-wider bg-gradient-to-r from-purple-400 to-cyan-400 bg-clip-text text-transparent">
            TRAVIS
          </span>
          <button
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
          {messages.length === 0 && !isLoading && (
            <div className="flex items-center justify-center h-full">
              <p className="text-gray-600 text-xs font-mono">메시지를 입력하세요...</p>
            </div>
          )}
          {messages.map((msg) => (
            <div
        ...
        <div className="px-3 py-3 border-t border-white/10">
          <div className="flex items-end gap-2">
            <textarea
              ...
              rows={1}
              disabled={isLoading}
              className="flex-1 bg-white/5 text-sm text-gray-200 placeholder-gray-600 px-3 py-2 rounded-lg border border-white/10 focus:border-purple-500/50 focus:outline-none resize-none font-mono disabled:opacity-50"
            />
            <button
              onClick={handleSend}
